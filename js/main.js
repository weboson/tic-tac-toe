// коротко:
// 1) разместили элементы: 
// пространство (area), саму клетку (cell), раздел "текущий игрок" (curPlyr) 
// 2.1) создали изначального игрока "x" (player)
// 2.2) создали изначальную статистику = 0
// 2.3) создали (разметили) выигрышные комбнации позиций
// 
// 3) в цикле повесили каждой ячейке событие "click"
// 4) создали обработчик событий:
//  "cellClick()" { 
//     проверка на занятые ячейки
    //    добавление в массив занятых ячеей
    //    if (проверк на выигрышные позиции) {
    //     статисткика = x/o + 1
    //     очищение после выигрыша
    //    } else {
    //       статистика = d + 1
    //         иначе Ничья!
    //    }
    //    чередорвание игроков
    //    player = player == "x" ? "o" : "x";
    //    вывод текущего пользователя на старницу
    //     currentPlayer.innerHTML = player.toUpperCase();
//  }
// 5) checkWin()  - Функция на проверку на совпадений путем циклов, где сравниваниются числа текущие с выигрышными
// 6) restart(text) - выводит текст (выиграл, ничья) на экран и заменяет занчения ячеек на пустые строки
// 7) updateStat() - функция вставки на страницу результатов статистики
...
function cellClick() {
    ...
    // условие (если ячейка НЕ занята, то вставить в ячейку текущего игрока "x" или "o":
    if (!this.innerHTML) {
        this.innerHTML = player; // == "x" или "o"
    } else {
        alert("Ячейка занята");
    ...
    for (let i in cell) {
        // если ячейка занята игроком (player == "x" или "o"), то добавить номер ячейки в массив data = [только "x"]
